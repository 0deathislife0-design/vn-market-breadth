"""Backtest Advanced Trailstop Buy signals.

For every historical ATS Buy signal, measure return from signal Close to Close
after T+10 sessions. A win is forward return >= 7%.
"""
...
OUTPUT_JSON = DATA_DIR / "backtest_advanced_trailstop.json"
DOCS_OUTPUT_JSON = DOCS_DATA_DIR / "backtest_advanced_trailstop.json"
...
def backtest_symbol(symbol: str) -> list[dict]:
    ...
    for i in np.flatnonzero(buy.to_numpy()):
        if i < 9 or i + LOOKFORWARD >= len(df):
            continue
        ...
        rows.append({
            ...
            "bs": round(float(signal["bs_series"].iloc[i]), 2),
        })
    return rows
...
def main():
    print("=" * 60)
    print("Backtest: Advanced Trailstop Buy T+10 >= 7%")
    print("=" * 60)
    ...
    now = vn_now()
    output = {
        "mode": "advanced_trailstop_buy_t10_7pct",
    ...
    print(f"All ATS Buy trades: {stats_all['wins']} / {stats_all['total']} = {stats_all['win_rate']:.1%}")
